...
//insertar-foto
  ...
  respuestaSwitch(res,id_usuario){
    ...
    if(res.error!=0){
      ...
      this.router.navigate(['recursosHumanos/usuarios/user-detail/'+id_usuario]);
    }
  }
  mostrar(){
  ...
  diplay(crearResp: CrearResp){
    ...
    if(crearResp.error==0){
      ...
      console.log(crearResp);
      setTimeout(()=>{ this.router.navigate(['recursosHumanos/insertar-foto/'+crearResp.id_usuario])}, 1500);
